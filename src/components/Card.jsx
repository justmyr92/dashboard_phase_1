        ...
        const fetchRecords = async () => {
            try {
                const response = await fetch(
                    `http://csddashboard.online//api/record/sdg/${sdg}/${instrument.instrument_id}`
                );
        ...
        const fetchRecordsData = async () => {
            try {
                const dataPromises = records.map(async (record) => {
                    const response = await fetch(
                        `http://csddashboard.online//api/record_data/approved/${record.record_id}/${ID}`
                    );
            ...
            try {
                ...
                for (const recordData of recordsData) {
                    for (const record of recordData) {
                        const response = await fetch(
                            `http://csddashboard.online//api/record_value/${record.record_data_id}`
                        );
                        const data = await response.json();
                        if (data.length > 0) {
                            temp.push(data);
